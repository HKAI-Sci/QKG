"""
Generate the main patient-context results figure from CSV.

This replaces the wide Table 1 with a compact two-panel bar plot aligned with
the existing result figure style and Figure 1 panel D color language.
"""
...
CSV_PATH = os.path.join(OUT_DIR, "main_patient_results.csv")

COLOR_BASE = "#9ecae1"   # no validator / baseline
COLOR_QKG = "#2171b5"    # QKG with patient
COLOR_NOPC = "#fd8d3c"   # KG without patient context
COLOR_IMP = "#41ab5d"
COLOR_REG = "#d73027"
...
def main() -> None:
    ...
    # Panel (a): final accuracy only, since the baseline is one of the plotted systems.
    bars = ax1.bar(x, final_acc, bar_w, color=colors, edgecolor="white", linewidth=0.8)
    for xi, (bar, acc, delta) in enumerate(zip(bars, final_acc, deltas)):
        ax1.text(bar.get_x() + bar.get_width() / 2, acc + 0.18, f"{acc:.1f}",
                 ha="center", va="bottom", fontsize=9, fontweight="bold")
        if delta > 0:
            ax1.text(bar.get_x() + bar.get_width() / 2, acc + 0.52, f"(+{delta:.1f} pp)",
                     ha="center", va="bottom", fontsize=8.2, color="#1f2937", fontweight="bold")

    ax1.set_xticks(x)
    ax1.set_xticklabels(labels, fontsize=9)
    ax1.set_ylabel("Final accuracy (%)", fontsize=10)
    ax1.set_title("(a) Final Accuracy", fontsize=11, fontweight="bold")
    ax1.set_ylim(74, 80.8)
    ax1.yaxis.set_major_formatter(plt.FuncFormatter(lambda v, _: f"{v:.0f}"))
    ax1.grid(axis="y", linestyle="--", alpha=0.5)
    ax1.spines[["top", "right"]].set_visible(False)

    # Panel (b): changed-answer counts as horizontal bars.
    ...
    for yi, (imp, reg) in enumerate(zip(improved, regressed)):
        if imp == 0 and reg == 0:
            ...
            continue
        ax2.text(imp + 3, yi - bar_w / 4, str(imp),
                 ha="left", va="center", fontsize=8.5)
        ax2.text(reg + 3, yi + bar_w / 4, str(reg),
                 ha="left", va="center", fontsize=8.5)
        net = imp - reg
    ...
    plt.suptitle("Haiku-Validator Results and Context Ablation", fontsize=12.5, fontweight="bold", y=1.01)
    plt.tight_layout()

    for ext in ("pdf", "png"):
        out_path = os.path.join(OUT_DIR, f"main_patient_results_plot.{ext}")
